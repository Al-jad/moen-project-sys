<template>
  <div 
    class="fixed top-0 right-0 z-50 flex flex-col h-screen bg-gray-900 shadow-lg group"
  ...
  >
      ...
      <ul class="mt-4 space-y-1.5">
        <template v-for="(item, index) in menuItems" :key="index">
          <li v-if="item.type === 'separator'" class="px-2">
            <hr class="w-full h-px my-2 border-0 rounded opacity-30 bg-gradient-to-l from-gray-700 to-transparent">
          </li>
          <li 
            v-else
          ...
          >
            <div 
              class="absolute inset-0 transition-opacity duration-200 rounded-md opacity-0 bg-gradient-to-l from-gray-800 via-transparent to-transparent group-hover/item:opacity-100"
    ...
    <div 
      class="relative flex items-center p-2 transition-all duration-300 ease-in-out border-t border-gray-800 cursor-pointer hover:bg-gray-800"
      :class="{'w-[60px]': !isAnyItemHovered, 'w-[220px]': isAnyItemHovered}"
    ...
    >
      <div class="flex items-center gap-3 px-2 py-1.5">
        <img 
          ...
          alt="User Avatar" 
          class="object-cover w-8 h-8 rounded-full ring-2 ring-gray-700"
        />
        <div 
          class="flex flex-col transition-all duration-300"
          :class="{'opacity-0': !isAnyItemHovered, 'opacity-100': isAnyItemHovered}"
        >
          <span class="text-sm font-medium text-white whitespace-nowrap">أ. دعاء الشيخلي</span>
          <span class="text-xs text-gray-400 whitespace-nowrap">مدير المشروع</span>
        </div>
        <LogOut 
